package cn.chiayhon.base.concurrent.twotermination;

public abstract class TerminateSupport extends Thread implements Terminatable {
    public final TerminationToken terminationToken;  // 记录当前的标志位

    protected TerminateSupport() {
        this(new TerminationToken());  // 初始化当前标志位
    }

    protected TerminateSupport(TerminationToken terminationToken) {
        super();
        this.terminationToken = terminationToken;  // 初始化标志位
        terminationToken.register(this);  // 注册当前对象的标志位
    }

    protected abstract void doRun() throws InterruptedException;  // 供子类实现具体任务的方法
